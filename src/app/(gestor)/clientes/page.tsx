    ...
    <div className="p-4 sm:p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6 sm:mb-8 gap-3">
        <div>
          ...
          <p className="text-slate-500 mt-0.5 text-sm">
            {clientsLoading ? "Carregando..." : `${clients.length} cliente${clients.length !== 1 ? "s" : ""} ativo${clients.length !== 1 ? "s" : ""}`}
          </p>
        </div>
      ...
      {clientsLoading ? (
        ...
        <div className="text-center py-24 text-slate-400">
          ...
          <p className="text-base font-medium text-slate-500 mb-1">Nenhum cliente cadastrado</p>
          <p className="text-sm mb-6">Crie o primeiro cliente para começar a gerenciar posts.</p>
          <Link href="/clientes/novo">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {clients.map((client) => {
            ...
            const published = clientPosts.filter((p) => p.status === "publicado").length
            return (
